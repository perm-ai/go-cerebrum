...
func (r Relu) Forward(input []*ckks.Ciphertext, inputLength int) []*ckks.Ciphertext {
	...
	for i, inputEach := range input {
		...
		// calculate degree 4
		xSquared := r.U.MultiplyNew(*inputEach.CopyNew(), *inputEach.CopyNew(), true, false)
		...
		deg1 := r.U.MultiplyPlainNew(inputEach.CopyNew(), &deg1coeff, true, false)
...
func (r Relu) Backward(input []*ckks.Ciphertext, inputLength int) []*ckks.Ciphertext {
	...
	for i, inputEach := range input {
		...
		xAndDeg3coeff := r.U.MultiplyPlainNew(inputEach.CopyNew(), &deg3coeff, true, false)
