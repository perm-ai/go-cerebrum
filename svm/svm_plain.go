...
type SvmPlain struct {
	weights []float64
}

func NewSVMModel() SvmPlain {
	weights := array.GeneratePlainArray(0, 4)
	return SvmPlain{weights: weights}
}

func (model *SvmPlain) UpdateSVMGradients(ascend []float64, l_rate float64, numOfFeatures int) {
	for i := 0; i < numOfFeatures; i++ {
		model.weights[i] = model.weights[i] - (ascend[i] * l_rate)
	}
}

func (model *SvmPlain) ComputeCostGradient(data [][]float64, target []float64, numOfFeatures int, regularizationStrength float64) []float64 {
...
func (model *SvmPlain) TrainSVM(data [][]float64, target []float64, epoch int, l_rate float64, numOfFeatures int, regularizationStrength float64) []float64 {
